  ...
  return (
    <div className="page-shell py-10">
      <section className="rounded-3xl border border-white/10 bg-cardLuxury/70 px-5 py-8 sm:px-8 lg:px-10">
        <p className="section-kicker">Offres Bouri Events</p>
        <h1 className="section-title mt-3">Choisissez le pack adapte a votre ambition</h1>
        <p className="section-subtitle mt-4">
          Trois niveaux premium pour piloter votre evenement avec style, structure et serenite.
        </p>
                ...
                >
                  {featured ? (
                    <p className="mb-4 inline-flex w-fit items-center gap-1 rounded-full border border-goldLuxury/35 bg-goldLuxury/12 px-3 py-1 text-xs uppercase tracking-[0.12em] text-goldLuxury">
                      <Sparkles className="h-3.5 w-3.5" /> Le plus demande
                    </p>
                  ) : null}

                  <p className="text-xs uppercase tracking-[0.14em] text-goldLuxury">{pack.price}</p>
                  ...
                  <div className="mt-7">
                    <Button asChild fullWidth variant={featured ? 'primary' : 'secondary'}>
                      <Link to="/register">Demarrer avec ce pack</Link>
                    </Button>
                  </div>
                </Card>
              </motion.div>
